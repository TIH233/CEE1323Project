...
# Ensure the response variable is treated as a factor for classification
...
top_airport_features <- names(flights_df_clean)[grepl("dep_|arr_", names(flights_df_clean))]
other_features <- setdiff(names(flights_df_clean), c("dep_airport_code", "dest_airport_code", "flight_num", "total_delay"))
model_formula <- reformulate(c(top_airport_features, other_features), response = "delay_class")

# Set seed for reproducibility
set.seed(123)

# Train a Random Forest model with the cleaned data
rf_model <- randomForest(model_formula, data = flights_df_clean, importance = TRUE, ntree = 300)

# Extract and sort feature importance in descending order
...
ggplot(importance_df, aes(x = reorder(feature, MeanDecreaseAccuracy), y = MeanDecreaseAccuracy)) +
  ...
  labs(x = 'Feature', y = 'Importance', title = 'Feature Importance in Predicting Delay Class') +
  theme_minimal()

# since the objective is solely get insights for importance, there is little need to tune the model
# since the importance measures are derived from the tree structures within the model 
# ranger() library is said to be able to compute quicker
# # use 
# # Register parallel backend to use multiple cores
# numCores <- detectCores()
# cl <- makeCluster(numCores - 1)  # leave one core free for system processes
# registerDoParallel(cl) to have parallel computing
